...
def init():
    ...
    onnx_path = "./onnx/out/yolov5s_2.0.onnx"
    if not os.path.isfile(onnx_path):
        log.error(f'{onnx_path} deos not exist')
        return None
    ie = Core()
    model_onnx = ie.read_model(model=onnx_path)
    
    log.info('Loading Inference Engine')
    net = compiled_model_onnx = ie.compile_model(model=model_onnx, device_name="CPU")

    input_blob = next(iter(net.inputs))
    n, c, h, w = input_blob.shape
    print(f'input blob shape: {n}, {c}, {h}, {w} (n, c, h, w)')
...
#     log.info(f'process_image, ({input_image.shape}')
    ...
    t = time.time()
    output_layer_onnx = net.output(0)

    # Run inference on the input image.
    res = net({input_blob: images})  # [output_layer_onnx]
    ...
    if data[0]==None:
        ...
        for idx,proposal in enumerate(data):
            if proposal[4] > 0 :
                ...
                xmin = int(iw * (proposal[0]/640))
                ymin = int(ih * (proposal[1]/640))
                xmax = int(iw * (proposal[2]/640))
                ymax = int(ih * (proposal[3]/640))
                idx = int(proposal[5])
...
if __name__ == '__main__':
    ...
    print("Average inference time (s):", (time.time()-t)/n)
